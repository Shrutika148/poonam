...
buffer[i]='\0';
execve("child1",buffer,NULL);/*executes an existing file*/
return 0;
}
...
while(list[i] <= list[pivot] && i <= high)//increment iwhen pivot is greater
{
i++;
}
while(list[j] > list[pivot] && i >= low)//decrement j when pivot is greater
{
j--;
}
